...
# ----------------------------------------------------------------------
# The default for this requirement is that the setting is not required, which an absent rule
# already satisfies, so evaluating every requirement reports no failure here.
@pytest.mark.parametrize("response", [[], [_OTHER_RULE]])
def test_SucceedsWithoutPullRequestRuleWhenEvaluatingAll(response):
...
def test_EvaluatedWithoutPullRequestRuleWhenEvaluatingAll(response):
    result = _Evaluate(response, require=True, evaluate_all=True)
...
def test_ResolutionEnablesPullRequestRuleWhenEvaluatingAll():
    result = _Evaluate([], require=True, evaluate_all=True)
...
def test_ErrorResolution():
    ...
    assert result.resolution == textwrap.dedent(
        ...
        4) Clear the **Require approval of the most recent reviewable push** checkbox.
...
def test_ErrorResolutionWhenRequired():
    ...
    assert result.resolution == textwrap.dedent(
        ...
        2) Click the name of the ruleset that targets `main`.
        3) Click **Show additional settings** beneath **Require a pull request before merging** in the **Branch rules** section.
        4) Check the **Require approval of the most recent reviewable push** checkbox.
        5) Click the **Save changes** button at the bottom of the page.
